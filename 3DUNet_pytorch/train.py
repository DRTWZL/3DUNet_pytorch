...
def val(model, val_loader, loss_func, n_labels):
    ...
    with torch.no_grad():
        for idx,(data, target) in tqdm(enumerate(val_loader),total=len(val_loader)):
            ...
            val_dice.update(torch.softmax(output, 1), target)
...
def train(model, train_loader, optimizer, loss_func, n_labels, alpha):
    ...
    for idx, (data, target) in tqdm(enumerate(train_loader),total=len(train_loader)):
        ...
        train_dice.update(torch.softmax(output, 1), target)
...
if __name__ == '__main__':
    ...
    loss = loss.DiceBceLoss(num_classes = args.n_labels)
    ...
    val_alpha = 0.9 # alpha * old + (1-alpha) * new
    val_cumulate_dice = 0
    while(epoch_alpha>0):
        epoch_alpha-=20
        alpha*=0.5
    for epoch in range(1, args.epochs + 1):
        ...
        average_dice = 0
        for dsc in val_log["Val_dice"]:
            average_dice = average_dice + dsc
        average_dice = average_dice / (len(val_log['Val_dice']))
        if average_dice > best:
            print('Saving best model')
            torch.save(state, os.path.join(save_path, 'best_model.pth'))
            trigger = 0
        print('Best performance at Epoch: {} | {}'.format(epoch,best))

        # 深监督系数衰减
        if epoch % 20 == 0: alpha *= 0.5
